...
void Fill_field(QPainter* p, QImage* image, QPixmap* map,int cellPosX, int cellPosY, int onePixDistance, int TxPower, int antGain, float freq)
{
    ...
    for(int i = 0; i < maxX; i++)
    {
        for(int j = 0; j < maxY; j++)
        {
            ...
            {
                p->setPen(QColor(242, 70, 0, 255)); //
            }
            if (sigPower < -59 && sigPower >= -64)
            {
                p->setPen(QColor(228, 115, 0, 255)); // красноорнжевый
            }
            if (sigPower < -64 && sigPower >= -69)
            {
                p->setPen(QColor(228, 145, 0, 255)); //
            }
